...
class Unit2Control(nn.Module):
    ...
    def forward(self, units, source, noise, volume, g):
        ...
        if self.n_spk is not None and self.n_spk > 1:
            ...
            g = g.squeeze(1).transpose(0, 1) # [B, H, N]
            x = x + g

        x = self.decoder(x)
        x = self.norm(x)
        e = self.dense_out(x)
        return split_to_(e, self.output_splits)
...
class CombSubSuperFast(torch.nn.Module):
    ...
    def fast_source_gen(self, f0_frames):
        n = torch.arange(self.bs, device=f0_frames.device)
        s0 = f0_frames / self.sr
        ds0 = F.pad(s0[:, 1:, :] - s0[:, :-1, :], (0, 0, 0, 1))
        rad = s0 * (n + 1) + 0.5 * ds0 * n * (n + 1) / self.bs
        s0 = s0 + ds0 * n / self.bs
    ...
    def forward(self, units_frames, mel2ph, f0_frames, volume_frames, g=None):
        '''
            units_frames: B x n_frames x n_unit
            f0_frames: B x n_frames
            volume_frames: B x n_frames
        '''
        decoder_inp = F.pad(units_frames, [0, 0, 1, 0])
        mel2ph_ = mel2ph.unsqueeze(2).repeat([1, 1, units_frames.shape[-1]])
        units_frames = torch.gather(decoder_inp, 1, mel2ph_)
        
        f0_frames_inp = f0_frames.unsqueeze(-1)
        f0_frames_inp[f0_frames_inp<self.f0_min]=self.f0_min
        volume_frames = volume_frames.unsqueeze(-1)

        combtooth = self.fast_source_gen(f0_frames_inp)
        ...
        return self.norm_spec(mel).unsqueeze(1), mel, f0_frames_inp.squeeze(-1)
...
def export_onnx(model_path, output_path):
    ...
    hu = torch.randn((1, 50, args.data.encoder_out_channels))
    mel2ph = torch.arange(0, 50).long().unsqueeze(0)
    f0 = torch.randn(1, 50)
    vol = torch.randn(1, 50)
    ...
    if n_spk is not None and n_spk > 1:
        ...
        test_sid = test_sid.repeat(50, 1)
        model.ddsp_model.unit2ctrl.export_chara_mix(n_spk)
        outtest = model.ddsp_model(hu, mel2ph, f0, vol, test_sid)
        torch.onnx.export(
            model.ddsp_model,
            (hu, mel2ph, f0, vol, test_sid),
        ...
        torch.onnx.export(
            ...
            opset_version=18,
            verbose=False,
            input_names=["hubert", "mel2ph", "f0", "volume"],
            output_names=["x", "cond", "f0_pred"]
        )
